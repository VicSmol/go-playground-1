...
### 6.3. Spec-First для SQL-запросов: sqlc

**sqlc** — это мощный инструмент для Go, который является квинтэссенцией подхода Spec-First для взаимодействия с базами данных. Вместо того чтобы писать SQL-запросы в Go-коде, вы описываете их в чистом `.sql` файле. Этот файл и становится вашей **спецификацией** для контракта с БД.

**Как это работает:**
1.  **Напишите спецификацию (SQL-файл):** Вы определяете все свои запросы (SELECT, INSERT, UPDATE, DELETE) в `.sql` файле, используя специальные комментарии-аннотации `-- name:`, чтобы sqlc знал, как генерировать код.
```sql
-- query.sql
-- name: CreateUser :one
...
```
2.  **Сгенерируйте код:** Запустите `sqlc generate`. Инструмент проанализирует ваш SQL и схему БД и сгенерирует типобезопасный Go-код.
3.  **Используйте сгенерированный код:** В вашем приложении вы вызываете сгенерированные методы, которые принимают и возвращают Go-структуры.
```go
func main() {
	// ... инициализация db
	queries := db.New(db)
	...
	// user — это обычная Go-структура
}
```

**Почему sqlc — это Spec-First:**
*   **Источник истины:** SQL-файл — это спецификация. Он определяет контракт.
*   **Гарантированная согласованность:** Если вы измените схему БД, sqlc выдаст ошибку при генерации, если ваши SQL-запросы больше не валидны, что вынуждает обновить спецификацию.
*   **Типобезопасность:** Сгенерированные структуры и методы исключают ошибки передачи параметров или обработки результатов.
*   **Автоматизация:** Легко интегрировать в CI/CD для проверки валидности запросов при каждом коммите.

**Преимущества перед ORM:**
*   **Полный контроль над SQL:** Вы пишете оптимальный SQL, а не полагаетесь на генератор ORM.
*   **Отличная производительность:** Нет накладных расходов ORM.
*   **Предсказуемый output:** Вы всегда знаете, какой запрос будет выполнен.
